...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint('row constraint', row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.append(AllDiffConstraint('column constraint', scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append(AllDiffConstraint('subsquare constraint', scope))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    vars = []
    vars_separated = []
    for i in range(0, len(planes_problem.planes)):
        flyable = planes_problem.can_fly(planes_problem.planes[i])
        startable = planes_problem.can_start(planes_problem.planes[i])
        num_flights = len(flyable)
        num_starts = len(startable)
        flyable.append("Unassigned")
        startable.append("Unassigned")
        flight_var = []
        if num_starts >= 1:
            flight_var.append(Variable(planes_problem.planes[i] + "Flight 1", startable))
        else:
            flight_var.append(Variable(planes_problem.planes[i] + "Flight 1", ["Unassigned"]))
            continue
        if num_flights > 1:
            for j in range(2, num_flights + 1):
                flight_var.append(Variable(planes_problem.planes[i] + "Plane Flight{}".format(j), flyable))
        vars.extend(flight_var)
        flight_var.insert(0, planes_problem.planes[i])
        vars_separated.append(flight_var)

    cons = []
    
    every_flight_scheduled = []
    for flight in planes_problem.flights:
        constraint = NValuesConstraint(flight, vars, [flight], 1, 1)
        every_flight_scheduled.append(constraint)

    maintenance = []
    k = planes_problem.min_maintenance_frequency
    maintenance_flights = planes_problem.maintenance_flights + ["Unassigned"]
    count = 1
    str = "plane " + "{}".format(count)
    for plane_vars in vars_separated:
        if len(plane_vars) > 1:
            num_spots = len(plane_vars) - 1
            if num_spots < k:
                continue
            for i in range(1, num_spots - (k-2)):
                vars_list = [plane_vars[i]]
                for j in range(i + 1, i + k):
                    vars_list.append(plane_vars[j])
                constraint = NValuesConstraint(str + ' flight {} to {}'.format(i, j), vars_list, maintenance_flights, 1, k)
                maintenance.append(constraint)
            count += 1
            str = "plane " + "{}".format(count)
    ...
    for plane_vars in vars_separated:
        if len(plane_vars) > 2:
            for i in range(1, len(plane_vars) - 1):
                vars_list = [plane_vars[i], plane_vars[i + 1]]
                satisfying = []
                for dom1 in plane_vars[i].domain():
                    satisfying.append([dom1, "Unassigned"])
                    for dom2 in plane_vars[i + 1].domain():
                        if (dom1, dom2) in can_follow:
                            satisfying.append([dom1, dom2])
                constraint = TableConstraint(str + ' flight{} follows {}'.format(i+1, i), [plane_vars[i], plane_vars[i+1]], satisfying)
                follow_cons.append(constraint)
            count += 1
            str = "plane " + "{}".format(count)

    cons.extend(every_flight_scheduled)
    cons.extend(maintenance)
    cons.extend(follow_cons)


    csp = CSP("{}Planes {}Flights".format(len(planes_problem.planes), len(planes_problem.flights)), vars, cons) #set to to your CSP 
    ...
    for sol in solutions:
        one_sol = []
        for plane in vars_separated:
            plane_sol = [plane[0]]
            for var in plane[1:]:
                for combi in sol:
                    if var == combi[0] and combi[1] != "Unassigned":
                        plane_sol.append(combi[1])
            one_sol.append(plane_sol)
        all_sol.append(one_sol)
    return all_sol
